	...
	private RuntimeWiring buildWiring() {
		...
		return RuntimeWiring.newRuntimeWiring()
				.type("Query", typeWriting -> typeWriting.dataFetcher("getAllPerson", fetcher1)
						.dataFetcher("findPerson", fetcher2).dataFetcher("findPersonByName", fetcher3))
				.build();
...
// using this single method get required data by preparing diffrent type script queries
	// following two methods data also we can fetch the data using corresponding
	// type script query
	@PostMapping("/getAll")
